...
# Modelos de nube para cada categoría
cloud_speed_low = CloudModel(0, 15, 3)
cloud_speed_medium = CloudModel(50, 15, 3)
cloud_speed_high = CloudModel(100, 15, 3)

cloud_occ_low = CloudModel(0, 15, 3)
cloud_occ_medium = CloudModel(50, 15, 3)
cloud_occ_high = CloudModel(100, 15, 3)

cloud_cong_low = CloudModel(0, 15, 3)
cloud_cong_medium = CloudModel(50, 15, 3)
cloud_cong_high = CloudModel(100, 15, 3)
...
df["fuzzy_predicted_label"] = df["fuzzy_output"].apply(classify_fuzzy_output)
df["cloud_predicted_label"] = df["cloud_output"].apply(classify_fuzzy_output)

# Métricas y visualización comparativa
cm_fuzzy = confusion_matrix(df["true_label"], df["fuzzy_predicted_label"])
cm_cloud = confusion_matrix(df["true_label"], df["cloud_predicted_label"])

report_fuzzy = classification_report(df["true_label"], df["fuzzy_predicted_label"], target_names=["Low", "Medium", "High"])
report_cloud = classification_report(df["true_label"], df["cloud_predicted_label"], target_names=["Low", "Medium", "High"])
...
fig_fuzzy.savefig("confusion_matrix_fuzzy.png")
...
fig_cloud.savefig("confusion_matrix_cloud.png")
